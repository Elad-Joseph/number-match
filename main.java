...
public class main {
    ...
    public void getBoard(){
        ...
        boardNumbers = cb.constractBoard();
    }
    public void game() {
        getBoard();
                ...
                if (clickcounter > 0) {
                    ...
                    if (clickcounter == 0)
                        if (usedbuttons[1] == usedbuttons[2]) {
                            ...
                            score += addedScore();
                            ;
                            numOfVisiblenumbersInRow[ButtonDataMap.get(usedbuttons[1]).Y] --;
                            numOfVisiblenumbersInRow[ButtonDataMap.get(usedbuttons[2]).Y] --;
                            score += addedScore;
                            scorepanel.setText(Integer.toString(score));
                            clickcounter = 2;
                            buttonsleft -= 2;
    ...
    public void findLastPos(){
        ButtonData button1 = (ButtonDataMap.get(usedbuttons[1]).Y > ButtonDataMap.get(usedbuttons[2]).Y||ButtonDataMap.get(usedbuttons[1]).X>ButtonDataMap.get(usedbuttons[2]).X) ? ButtonDataMap.get(usedbuttons[1]) : ButtonDataMap.get(usedbuttons[2]);
        ButtonData button2 = (ButtonDataMap.get(usedbuttons[1]).Y > ButtonDataMap.get(usedbuttons[2]).Y||ButtonDataMap.get(usedbuttons[1]).X<ButtonDataMap.get(usedbuttons[2]).X) ? ButtonDataMap.get(usedbuttons[1]) : ButtonDataMap.get(usedbuttons[2]);    
        
        if(ButtonDataMap.get(button1).X == lastButtonPos[1] && ButtonDataMap.get(button1).Y == lastButtonPos[0]){
            changeLastPos(button2);
        }
    }

    public void changeLastPos(ButtonData button){
        while(!ButtonDataMap.get(button).Visablilty){
